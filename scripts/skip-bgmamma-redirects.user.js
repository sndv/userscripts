...
document.querySelectorAll(LINK_CSS_SELECTOR).forEach(el => {
  try {
    if (el.href.includes(REDIRECT_URL_IDENTIFIER)) {
      ...
      let targetEncoded = url.searchParams.get(URL_PARAM_NAME);
      if (targetEncoded) {
        let targetFull = decodeURIComponent(atob(targetEncoded));
        if (targetFull.includes(LINK_SPLIT_PATTERN)) {
          let target = targetFull.split(LINK_SPLIT_PATTERN, 1)[0];
